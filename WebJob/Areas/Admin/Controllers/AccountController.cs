        ...
        public JsonResult Delete(string id)
        {
            ...
            using (var transaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    var candidateProfile = db.CandidateProfiles.FirstOrDefault(p=>p.UserId.Equals(user.Id));
                    if (candidateProfile != null)
                    {
                        var jobApplications = db.JobApplications
                                            .Where(ja => ja.Applications != null && ja.Applications.CandidateProfileID == candidateProfile.ID)
                                            .ToList();
                    ...
                    foreach (var verification in employerVerification)
                    {
                        ...
                        var jobs = db.Jobs.Where(j => j.UserId == verification.UserId).ToList();
