...
namespace WebJob.Areas.Employer.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            {
                // Đăng xuất người dùng hiện tại trước khi đăng nhập tài khoản mới
            ...
            if (user != null && await UserManager.CheckPasswordAsync(user, model.Password))
            {
                ...
                switch (result)
                {
                    case SignInStatus.Success:
                        ...
                        else if (await UserManager.IsInRoleAsync(user.Id, "Candidate"))
                        {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (UserManager.Users.Any(u => u.PhoneNumber == model.Phone))
                {
                ...
                if (result.Succeeded)
                {
                    // Gán role "Employer" cho user
                    var roleResult = await UserManager.AddToRoleAsync(user.Id, "Employer");

                    if (!roleResult.Succeeded)
                    {
                        // Nếu gán role thất bại, xóa user vừa tạo và thông báo lỗi
        ...
        {
            var exists = UserManager.Users.Any(u => u.Email == email);
            return Json(!exists);
        }
        [HttpPost]
        [AllowAnonymous]
        public async Task<JsonResult> CheckPhoneExists(string phone)
        {
            var exists = UserManager.Users.Any(u => u.Phone == phone);
            return Json(!exists);
        }
